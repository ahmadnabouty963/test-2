...
create or replace function public.can_access_conversation(conversation_uuid uuid) returns boolean language sql stable security definer set search_path='' as $$
  select exists(select 1 from public.conversations c where c.id=conversation_uuid and (c.user_id=auth.uid() or c.moderator_id=auth.uid() or public.is_moderator())); $$;
...
declare selected public.conversations; begin if not public.is_moderator() then raise exception 'Moderator access required'; end if;
update public.conversations set moderator_id=auth.uid(),status='active',accepted_at=now() where id=conversation_uuid and status='waiting' returning * into selected;
...
begin
  if not exists(select 1 from public.conversations c where c.id=conversation_uuid and (c.user_id=auth.uid() or c.moderator_id=auth.uid())) then raise exception 'Conversation access required'; end if;
...
create policy "read profiles" on public.profiles for select to authenticated using(id=auth.uid() or public.is_moderator());
create policy "update own name" on public.profiles for update to authenticated using(id=auth.uid()) with check(id=auth.uid() and role='user');
create policy "read participant conversations" on public.conversations for select to authenticated using(user_id=auth.uid() or moderator_id=auth.uid() or public.is_moderator());
create policy "create own conversation" on public.conversations for insert to authenticated with check(user_id=auth.uid() and moderator_id is null and status='waiting');
create policy "participants read messages" on public.messages for select to authenticated using(public.can_access_conversation(conversation_id));
create policy "participants send messages" on public.messages for insert to authenticated with check(sender_id=auth.uid() and public.can_access_conversation(conversation_id) and exists(select 1 from public.conversations c where c.id=conversation_id and c.status<>'closed'));
grant execute on function public.accept_conversation(uuid) to authenticated; grant execute on function public.close_conversation(uuid) to authenticated; grant execute on function public.is_moderator() to authenticated; grant execute on function public.can_access_conversation(uuid) to authenticated;
